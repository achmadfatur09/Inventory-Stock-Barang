<tr class="row-keranjang">
	<td class="nama_barang">
		<?= $this->input->post('nama_barang') ?>
		<input type="hidden" name="nama_barang_hidden[]" value="<?= $this->input->post('nama_barang') ?>">
	</td>
	<td class="kode_barang">
		<?= $this->input->post('kode_barang') ?>
		<input type="hidden" name="kode_barang_hidden[]" value="<?= $this->input->post('kode_barang') ?>">
	</td>
	<td class="warna">
		<?= $this->input->post('warna') ?>
		<input type="hidden" name="warna_hidden[]" value="<?= $this->input->post('warna') ?>">
	</td>
	<td class="ukuran_kaos">
		<?= $this->input->post('ukuran_kaos') ?>
		<input type="hidden" name="ukuran_kaos_hidden[]" value="<?= $this->input->post('ukuran_kaos') ?>">
	</td>
	<td class="jumlah">
		<?= $this->input->post('jumlah') ?>
		<input type="hidden" name="jumlah_hidden[]" value="<?= $this->input->post('jumlah') ?>">
	</td>
	<td class="satuan">
		<?= strtoupper($this->input->post('satuan')) ?>
		<input type="hidden" name="satuan_hidden[]" value="<?= $this->input->post('satuan') ?>">
	</td>
	<td class="aksi">
		<button type="button" class="btn btn-danger btn-sm" id="tombol-hapus" data-nama-barang="<?= $this->input->post('nama_barang') ?>" data-barang="<?= $this->input->post('nama_barang')." ".$this->input->post('warna')." ".$this->input->post('ukuran_kaos') ?>"><i class="fa fa-trash"></i></button>
	</td>
</tr>

